...
def plot_pd_trajs(trajectory):
    """Plot key components of a trajectory.

    This function visualizes the states, rewards, and actions from a trajectory DataFrame
    and saves the plots as images in the './plot/' directory.

    
    :param trajectory: A DataFrame containing trajectory data
    :return:
    """
...
def collect_step(environment, control_function):
    """Collect a single step of interaction between the environment and the control function.

    :param environment: environment object
    :param control_function: A function that takes the current observation and time 
        and returns an action.

    
    :return: a tuple of `(time_step, action, next_time_step)`:
            - `time_step`: The initial state before taking the action.
            - `action`: The action generated by the control function.
            - `next_time_step`: The resulting state after executing the action.
    """
...
def collect_data(env, control_function):
    """ Collect an entire episode of data from the environment using the control function.
    ...
    :return: a tuple of four lists:
            - `states` (list): Observations collected at each step.
            - `rewards` (list): Rewards collected at each step.
            - `actions` (list): Actions taken at each step.
            - `times` (list): The timestamps of each step in the environment.
    """
