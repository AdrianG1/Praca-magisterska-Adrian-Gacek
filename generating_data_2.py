...
def plot_df_trajs(trajs):
    """ Plot kontrolny generowanej trajektorii"""
    plt.figure()
    plt.plot(trajs[['T', "T_błąd"]])
    plt.title('losses')
    plt.savefig('./plot/states_trajs.png')
    plt.figure()
    plt.plot(trajs["Akcje"])
...
def main(argv=None):
    ...
    plot_df_trajs(trajectory)
    trajectory.to_csv("./csv_data/trajectory1.csv")
