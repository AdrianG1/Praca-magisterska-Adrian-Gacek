...
def main():
    ...
    clk = tclab.clock(99999999999, step=0.1)
    ...
    log_memory_usage()
    for i in tqdm(range(30, 35, 775)):
        sum_diff = 0
        try:
            policy_path = (f'./policies/DQN_2_{i}')
            loaded_policy1 = tf.saved_model.load(policy_path)
            loaded_policy2 = tf.saved_model.load(policy_path)
            policy_state1 = loaded_policy1.get_initial_state(batch_size=1)
            policy_state2 = loaded_policy2.get_initial_state(batch_size=1)
            ...
            while not time_step1.is_last():
                ...
                action_step1 = loaded_policy1.action(time_step1, policy_state1)
                policy_state1 = action_step1.state
                if env1.envs[0].time > 2800 and env1.envs[0].time <3000 and real_state1[0, 1] == 30:
                    time_step_1 = env1.step(0)
                else:
                    time_step_1 = env1.step(action_step1.action)
                time_step1 = ts.TimeStep(
                ...
                # druga grzałka
                action_step2 = loaded_policy2.action(time_step2, policy_state2)
                policy_state2 = action_step2.state
                time_step_2 = env2.step(action_step2.action)
                time_step2 = ts.TimeStep(
                ...
                controls[0].append(float(action_step1.action))
                controls[1].append(float(action_step2.action))
