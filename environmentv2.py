...
class Environment(py_environment.PyEnvironment):
    ...
    def __init__(self, discret=False):
        ...
        self.clk = tclab.clock(self.EPISODE_TIME+200)

        # inicjalizacja generatora nastaw
        self._T_gen = setpoint_gen(self.clk)

        # inicjalizacja pamięci
        self.memory = deque(maxlen=self.MEM_LEN)
    ...
    def __gen_spec(self):
        ...
        if self.discret:
            ...
            self._action_spec = array_spec.BoundedArraySpec(
                shape=(), dtype=np.float32, minimum=0, maximum=100, name='action')
